...
// Add ngrok bypass headers and CORS for public endpoints
app.use((req, res, next) => {
  res.setHeader('ngrok-skip-browser-warning', 'true');
  res.setHeader('ngrok-skip-browser-warning', 'any');
  req.headers['ngrok-skip-browser-warning'] = 'true';
  
  // CORS headers for public WhatsApp endpoints
  if (req.path === '/api/whatsapp/public-settings') {
...
async function validateSessionToken(req, res, next) {
  ...
  if (req.path.startsWith('/webhooks') || 
      ...
      req.path === '/api/whatsapp/public-settings' ||
      req.method === 'OPTIONS') {
    return next();
  }
